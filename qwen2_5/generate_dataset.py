...
remove_chars = ["ṃ", "γ", "ε"]  # Characters to remove from the dataset
keep_chars = ["ʷ"]  # Characters to keep
...
list_of_chars = [c for c in list_of_chars if not c.isascii() and c not in remove_chars]
special_amazigh_chars = set(list(map(lambda x:x.lower(),set(list_of_chars))) +list(map(lambda x:x.upper(),set(list_of_chars))))
special_amazigh_chars = sorted(special_amazigh_chars)
transcriptions_list_filtered = [text for text in transcriptions_list if any(c in special_amazigh_chars for c in text)]
print("Count of special leters in data",Counter([elem for elem in list("".join(transcriptions_list)) if elem in special_amazigh_chars ]) )
...
if __name__ == "__main__":
    ...
    for i,text in tqdm(enumerate(transcriptions_list_filtered)):
